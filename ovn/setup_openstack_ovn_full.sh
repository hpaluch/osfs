...
[ -f $STAGE ] || {
	sudo $APT_CMD install -y neutron-server neutron-plugin-ml2 \
		...
		ovn-host openvswitch-switch neutron-ovn-metadata-agent ovn-central
	# Disable and stop all services until they are properly configured to avoid eating CPU, etc...
	sudo systemctl disable --now neutron-server \
		ovn-northd.service neutron-ovn-metadata-agent.service
...
[ -f $STAGE ] || {
	...
	sudo crudini --set $f DEFAULT service_plugins ovn-router
	sudo crudini --set $f DEFAULT transport_url "rabbit://openstack:$rp@$HOST:5672/"
...
STAGE=$CFG_STAGE_DIR/070-neutron-ovn
[ -f $STAGE ] || {
	sudo systemctl enable --now ovn-controller neutron-ovn-metadata-agent
